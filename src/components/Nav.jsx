...
const Nav = () => {
  ...
  // Prevent body scrolling if nav is open.
  if (navOverlay) {
  ...
  const handleClick = () => {
    ...
    if (window.innerWidth <= 975) {
      setNavOverlay((prevValue) => !prevValue);
    }
  };
    ...
    <>
      <header className="sticky top-0 z-50 bg-primaryColor py-4 shadow-xl">
        <nav className="wrapper relative flex items-center justify-between">
          ...
          {/* Nav links */}
          <div
            className={`fixed top-0 z-50 h-full w-[85%] bg-grayColor lg:h-auto lg:w-auto lg:p-0 ${isExpanded ? "right-0" : "right-[-900px]"} px-8 py-24 transition-all duration-500 lg:static lg:bg-transparent`}
          >
                ...
                {isAuthenticated ? (
                  <NavLink to={"/profile"} onClick={handleClick}>
                    <div className="flex flex-col gap-1 duration-300 ease-in-out hover:text-secondaryColor sm:flex-row sm:items-center lg:gap-0">
                      ...
                      <p className="mt-1 text-lg lg:text-sm">{user.username}</p>
                    </div>
                  </NavLink>
                ) : (
      ...
      <div className={`overlays ${navOverlay && "show"}`}></div>
    </>
  );
};
